...
class Light:
  ...
  def __init__(self, light_id, settings):
    ...
    self.bridge_port  = settings.bridge_port
  ...
  #     (self.bridge_ip, self.bridge_port, self.bridge_user, self.light), data=data)
  ...
  def set_light2(self, hue, sat, bri, duration=None):
    ...
    self.request_url_put("http://%s:%s/api/%s/lights/%s/state" % \
      (self.bridge_ip, self.bridge_port, self.bridge_user, self.light), data=dataString)
...
class Group(Light):
  ...
  def get_lights(self):
    try:
      r = requests.get("http://%s:%s/api/%s/groups/%s" % \
        (self.bridge_ip, self.bridge_port, self.bridge_user, self.group_id))
      j = r.json()
    except:
  ...
  #     (self.bridge_ip, self.bridge_port, self.bridge_user, self.group_id), data=data)
  ...
  def set_light2(self, hue, sat, bri, duration=None):
    ...
    self.request_url_put("http://%s:%s/api/%s/groups/%s/action" % \
      (self.bridge_ip, self.bridge_port, self.bridge_user, self.group_id), data=dataString)
...
class ChapterManager:
  ...
  def CreditsStartTimeForMovie(title, t_duration=None, chapterCount=None):
    ...
    if t_duration != None:
      ...
      xbmc.log("%s: DEBUG %s" % ("Kodi Raspbee", "t_duration %r" % t_duration))
    ...
    for res_chapterInfo in root.findall("{%s}chapterInfo" % XML_NAMESPACE):
      ...
      xbmc.log("%s: DEBUG %s" % ("Kodi Raspbee", "selected chapterdb entry with duration %r" % res_duration.text))
